...
def detectInterval(score_plot_agg, peak, left_dis, right_dis,
                   threshold):  # dis = distance to left and right of the peak
    start = 0
    best_diff = 0
...
def spotting(dataset_name, final_subjects, final_videos, dataset, result, subject_count, micro_threshold,
             macro_threshold, dif_threshold, show=False):
    ...
    for videoIndex, video in enumerate(final_videos[subject_count - 1]):
        ...
        if show:
            print('\nSubject:', final_subjects[subject_count - 1], subject_count, 'Video:',
                  videos[countVideo + videoIndex], countVideo + videoIndex)
        ...
        for peak in peaks_micro:
            ...
            if end - start > micro_min and end - start < micro_max and (score_plot_micro[peak] > 0.95 or (
                    score_plot_micro[peak] > score_plot_micro[start] and score_plot_micro[peak] > score_plot_micro[
                end])):
        ...
        for peak in peaks_macro:
            ...
            if end - start > macro_min and end - start < macro_max and (score_plot_macro[peak] > 0.95 or (
                    score_plot_macro[peak] > score_plot_macro[start] and score_plot_macro[peak] > score_plot_macro[
                end])):
                macro_detected.append([start, peak, end])

        micro_detected = checkPeak(micro_detected, macro_detected, score_plot_micro, score_plot_macro, dif_threshold,
                                   k_micro)
        micro_list = []
        macro_list = []
